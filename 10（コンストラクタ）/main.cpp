...
void Map::print()
{
    for (int y = 0; y < YSIZE; y++)
    {
        for (int x = 0; x < XSIZE; x++)
        {
            if (game->player->cellPrint(x, y))
            {
                continue;
            }
            if (game->monster1->cellPrint(x, y))
            {
                continue;
            }
            if (game->monster2->cellPrint(x, y))
            {
                continue;
            }
            cellPrint(x, y);
...
bool Player::moveTo(int dx, int dy)
{
    ...
    if (game->map->data[targetX][targetY] == Map::ROCK)
    {
        return false;
    }
...
Monster::Monster(Game *game_, int x_, int y_)
{
    game = game_;
    x = x_;
    y = y_;
